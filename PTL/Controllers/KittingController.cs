...
namespace PTL.Controllers
{
    public class KittingController : Controller
    {
        ...
        private const string SerialLine = "COM14";
        private const string SerialLineT = "COM3";
        ...
        public double readWeight()
        {
            ...
            try
            {
                using (SerialPort serialPort = new SerialPort(SerialLine, BaudRate))
                {
                    serialPort.Open();
                    var weight = serialPort.ReadLine();
                    //serialPort.Close();
                    weight = weight.TrimStart();
                    string[] weightList = weight.Split('g');
        ...
        public void controlLight(int relay)
        {
            _serialPort = new SerialPort(SerialLineT, BaudRate);
            ...
            { 
                try
                {
                    using (SerialPort serialPort = new SerialPort(SerialLineT, BaudRate))
                    {
                        serialPort.Open();
                        serialPort.Write(new byte[] { 255, 1, 1 }, 0, 3);
                        Thread.Sleep(3000);
                        serialPort.Write(new byte[] { 255, 1, 0 }, 0, 3);
                    }
                }
                catch (Exception e)
                {
            ...
            }else
            {
                try
                {
                    ...
                    {
                        serialPort.Open();
                        serialPort.Write(new byte[] { 255, 2, 1 }, 0, 3);
                        Thread.Sleep(3000);
                        serialPort.Write(new byte[] { 255, 2, 0 }, 0, 3);
                    }
                }
                catch (Exception e)
                {
        ...
        [HttpPost]      
        public List<Part> GetDeficient( Product p)
        {
            try{
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    List<Part> deficient = new List<Part>();

                    foreach (var item in required)
                    {
                        foreach (var a in available)
                        {
                            if (item.PartName == a.PartName && item.Qty > a.Qty) {
                                deficient.Add(item);
                            }
                        }
